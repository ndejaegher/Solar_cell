...
mh_eff = [0.67, 0.67, 0.33]#-0.214 # in [m_e]


T_sample = np.linspace(40,320,100)
...
kx = np.linspace(-k_ref[0]/2,k_ref[0]/2,30)
ky = np.linspace(-k_ref[1]/2,k_ref[1]/2,30)
kz = np.linspace(-k_ref[2]/2,k_ref[2]/2,30)
...
# ### Electrons mobility of the $CH_3NH_3PbI_3$

# Experimental values for electrons mobility in thin film obtained by R.L. Milot et al. in [1] are
# - $150 cm^2 V^{−1} s^{−1}$ at $80 K$
# - $35 cm^2 V^{−1} s^{−1}$ at $300 K$
# $\newline$
# The values of the scattering rate $\tau$ fitting the curves with the experimental data are respectively
# - $0.86 fs$ at $80 K$
# - $0.31 fs$ at $300 K$
# $\newline$
# which is about 10 times smaller than the scattering rate in thin film obtained in [2]
...
real_carrier = [150, 35, 62, 40, 52, 44, 41, 39, 26]
temp = [80, 300, 120, 140, 160, 180, 205, 230, 265]
color = ["go","mo", "bo", "bo", "bo", "bo", "bo", "bo", "bo"]
